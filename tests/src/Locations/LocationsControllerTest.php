...
use HR\Locations\{ LocationsDto, LocationsModel, LocationsController };
...
class LocationsControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        $this->input = [
            ...
            "postal_code" => "company",
            "city" => "would",
            "state_province" => "accept",
            "country_code" => "enter",
        ];
        $this->dto = new LocationsDto($this->input);
        $this->model = new LocationsModel($this->dto);
        $this->service = $this->createMock("HR\Locations\ILocationsService");
    ...
    public function testGet_ReturnsResponse(): void
    {
        ...
        $args = ["location_id" => 2923];

        $this->service->expects($this->once())
            ->method("get")
            ->with($args["location_id"])
            ->willReturn($this->model);
